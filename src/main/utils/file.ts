...
import { audioExts, documentExts, HOME_M3AGENT_DIR, imageExts, MB, textExts, videoExts } from '@shared/config/constant'
...
export function getTempDir() {
  return path.join(app.getPath('temp'), 'M3Agent')
}
...
export function getConfigDir() {
  return path.join(os.homedir(), HOME_M3AGENT_DIR, 'config')
}
...
export function getMcpDir() {
  return path.join(os.homedir(), HOME_M3AGENT_DIR, 'mcp')
}
