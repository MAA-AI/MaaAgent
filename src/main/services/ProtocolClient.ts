...
export const M3AGENT_PROTOCOL = 'm3agent'

export function registerProtocolClient(app: Electron.App) {
  if (process.defaultApp) {
    if (process.argv.length >= 2) {
      app.setAsDefaultProtocolClient(M3AGENT_PROTOCOL, process.execPath, [process.argv[1]])
    }
  }

  app.setAsDefaultProtocolClient(M3AGENT_PROTOCOL)
}

export function handleProtocolUrl(url: string) {
  if (!url) return
  // Process the URL that was used to open the app
  // The url will be in the format: m3agent://data?param1=value1&param2=value2
...
const DESKTOP_FILE_NAME = 'm3agent-url-handler.desktop'

/**
 * Sets up deep linking for the AppImage build on Linux by creating a .desktop file.
 * This allows the OS to open m3agent:// URLs with this App.
 */
...
Terminal=false
Type=Application
MimeType=x-scheme-handler/${M3AGENT_PROTOCOL};
NoDisplay=true
`
